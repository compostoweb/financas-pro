...
import { Checkbox } from "@/components/ui/checkbox" // <--- Novo Componente
...
export function TransactionTable({ transactions, loading, onUpdate }: TransactionTableProps) {
  ...
  // Limpa a seleção quando a lista muda (ex: filtro de data)
  useEffect(() => {
    setSelectedIds([])
  }, [transactions])
  ...
  // Filtra apenas transações reais (ignora resumos virtuais do sócio)
  const realTransactions = transactions.filter(t => !t.id?.startsWith('summary-adriano'))

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      // Seleciona todos os IDs reais
  ...
  if (!Array.isArray(transactions)) return <div className="p-8 text-red-600">Erro ao carregar lista</div>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* BARRA DE AÇÕES FLUTUANTE (Aparece quando seleciona algo) */}
      {selectedIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-6 py-3 rounded-full shadow-xl flex items-center gap-6 animate-in slide-in-from-bottom-5 fade-in duration-300">
           <div className="text-sm font-medium">
             <span className="bg-red-700 px-2 py-0.5 rounded-full mr-2 text-xs">{selectedIds.length}</span>
             selecionado(s)
           </div>
           
           <div className="h-4 w-px bg-slate-700" /> {/* Divisória */}
              ...
              return (
                <TableRow 
                    ...
                    className={`
                        ...
                        ${isSelected ? 'bg-slate-50 hover:bg-slate-100' : 'hover:bg-slate-50'}
                    `}
                >
                  {/* COLUNA CHECKBOX (Linha) */}
                  <TableCell className="text-center">
                    {!isSummary ? (
                       ...
                       <div className="w-4 h-4" /> // Espaço vazio para alinhar
                    )}
                  </TableCell>
